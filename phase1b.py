		...
		try:
			if self.curr_pose.pose.position.z > 0:
				...
				mask = cv2.inRange(image_blur_hsv, min_red, max_red)
        			edged = cv2.Canny(mask, 200, 500,L2gradient = True ) # canny edge detector
				...
				if len(contours) != 0:
					...
					if M["m00"]!=0:   
					  ...
					  cv2.circle(self.pixel_img,(cX,cY-10),7,(0,0,255), -1)      
					  string = str(cX) + "," + str(cY-10)
					  cv2.putText(self.pixel_img,string,(cX,cY-10),font,0.5,(255,0,0),2) # put text
  			                y_a = 320-(cY-10)  # cY is a center of rover in Y direction, and 320 is the center location of drone.
			                x_a = 320-(cX)  #  cX is a center of rover in X direction, and 320 is the center location of drone.
			                self.prob_ang = np.arctan2(y_a,x_a) # formula to find angle between drone and rover
	...
	def ascend(self):
		...
		while self.mode=="ASCEND" and not rospy.is_shutdown():
			...
			if self.curr_pose.pose.position.z > 10:
				if set_ang is False:
					angle = quaternion_from_euler(0,0,self.prob_ang) # angle between rover and drone converted into quaternion. 
